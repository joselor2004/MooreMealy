...
\begin{prop} \label{stabRat}
    Les relations rationnelles sont stables par inverse et union. De plus les projections d'une relation
...
\begin{proof}
    ...
    relation $R^{-1}$.\\
    Pour obtenir l'union de relations rationnelles, on crée le transducteur ayant pour état initial $q_0$ et des transitions depuis $q_0$
étiquetées par $(\varepsilon \mid \varepsilon)$ vers des copies des transducteurs reconnaissant ces relations.\\
    Enfin, si l'on considère l'automate obtenu en ne gardant que les étiquettes d'entrée sur les transitions, cet automate reconnait bien $R_{\Sigma}$.
...
\begin{proof}
    ...
    \\~\\
    Cet automate implémente donc la relation binaire $[T]$ qui contient $((u, v), w)$ si et seulement si $(u, v)$ est dans la première relation,
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
$R_l$ étant reconnue par l'automate figure \ref{autoRl}, et $R_b$ par \ref{autoRb}.\\
On a alors que
...
\begin{figure}[h] 
    \caption{L'automate reconaissant la relation $R_l$}\label{autoRl}
    \begin{center}
    \begin{tikzpicture}[auto]
        \node (q0) [state, initial, initial text= ] {$q_0$};
        ...
        \path[-stealth, thick]
        (q0) edge [bend left] node {$a \mid \{a, b\}$} (q1)
        (q1) edge [bend left] node {$\varepsilon \mid \{a, b\}$} (q0)
...
\begin{figure}[h] 
    \caption{L'automate reconaissant la relation $R_b$}\label{autoRb}
    \begin{center}
    \begin{tikzpicture}[auto]
